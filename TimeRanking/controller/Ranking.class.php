...
class Controller_Ranking {
    ...
    private $tipoRanking, $ranking, $raca, $classe, $levelInicial, $levelFinal, $limite;

    public function __construct() {
        
    }

    public function setTipoRanking($tr) {
        $this->tipoRanking = $tr;
    }
    ...
    public function getRankingCharacterJSON() {
        if (in_array($this->tipoRanking, array(1, 2, 3, 4))) {
                        ...
                        if (ctype_alnum($this->raca)) {
                            if (ctype_digit($this->levelInicial) && ctype_digit($this->levelFinal)) {
                                try {
                                    ...
                                    if ($this->tipoRanking == 2) {
                                        $this->ranking .= '_diario';
                                    } else if ($this->tipoRanking == 3) {
                                         $this->ranking .= '_semanal';
                                    } else if ($this->tipoRanking == 4) {
                                         $this->ranking .= '_mensal';
                                    }                                    
                                    ...
                                    throw new Exception('Falha ao buscar Characters');
                                }
